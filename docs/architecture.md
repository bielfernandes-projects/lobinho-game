...
- **Tokens**: `assets/design-tokens.json` + `assets/design-tokens.css` authored manually (the `sync-brand-to-tokens.cjs` script references `.claude/skills/design-system/scripts/generate-tokens.cjs` which does not exist in this repo). Registered in `globals.css` `@theme` as Tailwind utilities: `primary`, `primary-hover`, `primary-active`, `primary-light`, `midnight`, `surface`, `surface-raised`, `moon-silver`, `accent`, `accent-light`, plus `--font-display` (Bebas Neue).
- **Logo**: deleted `branding/` directory (old 6 SVG variants, showcase, prompts). Rewrote `public/icon-192x192.svg` and `public/icon-512x512.svg` as geometric "L" paths (no system-font `<text>` dependency) and regenerated all 4 PNGs (`icon-*` + `icon-maskable-*`) via System.Drawing with exact pixel math matching the SVGs; maskable versions use full-bleed Midnight bg with the mark at 62% centered (safe-zone compliant).
- **App application**: home screen now shows the mark (`/icon-192x192.png`) + "LOBINHO" wordmark in Bebas Neue `tracking-[0.18em]`; buttons migrated to `bg-primary`/`hover:bg-primary-hover`/`active:bg-primary-active` tokens. Display font applied to: game phase header, card-reveal header, PIN display in lobby, day-announcement victim names + "Ninguém morreu", win/lose screen, accused-name banners (host panel + player view), flip-card role name, tribunal countdown timer. `layout.tsx` loads Bebas Neue via `next/font/google` and exposes `--font-bebas-neue`.
- **Microcopy audit**: tribunal, timers, lobby, and game-phase copy already conform to the brand voice — only typographic treatment changed. `manifest.json`/`themeColor` already `#0A0A0A` (Midnight) — no change needed.
- **Not changed by design**: `ROLE_STYLE` palette in `src/lib/cards.ts` (role chips keep their own 100/700/300 color families, they do NOT change with the brand theme); Pix button in `support-footer.tsx` keeps yellow (payment-trust convention in Brazil).
- **Verification**: `npm ci` was required to fix the OneDrive Files-On-Demand issue — `node_modules`, `.env.local` and `.next/cache` files were online-only placeholders the cloud provider could not hydrate (reads returned `UNKNOWN`/`ENOENT`), which made `next dev`/`next build` silently no-op. `npm ci` rewrote `node_modules` as real local files. `next build --webpack` then compiled successfully (webpack + TypeScript both passed); local build only fails at the `_not-found` prerender step because `.env.local` is still an unreadable placeholder (Supabase URL/key missing) — production is unaffected (env comes from the Vercel dashboard). `npm run lint` now runs: 186 problems (45 errors / 141 warnings) all pre-existing in files NOT touched by the brand work. `sharp` was unavailable for PNG rasterization (install script blocked by npm allow-scripts) — icons were generated via System.Drawing in PowerShell instead.
- **Note**: `npm approve-scripts` still pending for `sharp`/`unrs-resolver` postinstall (only relevant if image optimization / ESLint resolver scripts are needed later).
- **Files**: `docs/brand-guidelines.md`, `assets/design-tokens.json`, `assets/design-tokens.css`, `public/icon-{192,512}x{192,512}.{svg,png}`, `public/icon-maskable-*.png` (regenerated), `src/app/globals.css`, `src/app/layout.tsx`, `src/app/page.tsx`, `src/app/lobby/[id]/page.tsx`, `src/app/game/[id]/page.tsx`, `src/components/flip-card.tsx`, `src/components/day-announcement.tsx`, `src/components/vote-timer-panel.tsx`, `src/components/tribunal-panel.tsx`, `docs/architecture.md`.
- **Status**: NOT committed — user reviews locally first, then approves commit/push.
