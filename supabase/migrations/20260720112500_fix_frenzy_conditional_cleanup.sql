-- Fix: Wolf Cub dying within resolve_night (poison or frenzy kill)
-- immediately clears the wolves_frenzy flag it just set.
--
-- Bug: The trigger trg_wolf_cub_death sets rooms.wolves_frenzy = true
-- when a cub dies (line ~151/201/247), but the unconditional cleanup
-- at the end (old line ~359) clears it in the same function call.
-- Result: next night sees v_frenzy=false, wolves kill only 1.
--
-- Fix: wrap the cleanup in `IF v_frenzy THEN` — only clear if frenzy
-- was already active BEFORE this night (i.e. consumed). New cub deaths
-- mid-function set frenzy for the NEXT night and must survive.
...
BEGIN
  ...
  IF v_diseased_skip THEN
    ...
    -- Lobos pulam: sem vitimas, avanca para dia
    UPDATE game_state
  ...
  IF v_poison_target_id IS NOT NULL THEN
    ...
    IF v_killed_by_poison THEN
      ...
      -- CURSED por veneno: NAO converte (so lobos convertem)
      v_victims := v_victims || jsonb_build_object(
  ...
  -- B21 FIX: Agora sim — apos todas as mortes, promover Squire se Prince morreu
  ...
  -- DOPPELGANGER: quando alvo morre, copia o papel
  ...
  -- Limpar frenesi: SO se foi CONSUMIDO nesta noite (v_frenzy era true antes das mortes).
  -- Se o Filhote morreu DENTRO desta noite, o trigger trg_wolf_cub_death ja
  -- setou wolves_frenzy = true para a PROXIMA noite — nao podemos limpar.
  IF v_frenzy THEN
